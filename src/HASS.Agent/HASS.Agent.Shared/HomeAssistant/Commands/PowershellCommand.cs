...
public class PowershellCommand : AbstractCommand
{
    ...
    public override void TurnOn()
    {
        ...
        if (!executed)
            Log.Error("[POWERSHELL] [{name}] Executing {descriptor} failed", EntityName, _descriptor);
    ...
    public override void TurnOnWithAction(string action)
    {
        ...
        if (!executed)
            Log.Error("[POWERSHELL] [{name}] Launching PS {descriptor} with action '{action}' failed", EntityName,
                _descriptor, action);

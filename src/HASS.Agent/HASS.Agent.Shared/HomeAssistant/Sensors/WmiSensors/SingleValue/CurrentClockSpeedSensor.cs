...
/// <summary>
/// Sensor indicating the CPU's (advertised) clock speed
/// </summary>
public class CurrentClockSpeedSensor(
    int? updateInterval = null,
    string? entityName = CurrentClockSpeedSensor.DefaultName,
    string? name = CurrentClockSpeedSensor.DefaultName,
    string? id = default,
    bool applyRounding = false,
    int? round = null,
    string? advancedSettings = default)
    : WmiQuerySensor(string.Empty,
        string.Empty,
        applyRounding,
        round,
        updateInterval ?? 300,
        ...
        advancedSettings: advancedSettings)
{
    ...
    private readonly ManagementObject _managementObject = new("Win32_Processor.DeviceID='CPU0'");
               ...
               SetAutoDiscoveryConfigModel(
                   new SensorDiscoveryConfigModel
                   {
                       ...
                       State_topic = $"{Variables.MqttManager.MqttDiscoveryPrefix()}/" +
                                     $"{Domain}/{deviceConfig.Name}/{ObjectId}/state",
    ...
    public override string GetState()
    {
        try
        {
            ...
            if ((DateTime.Now - LastFetched).TotalHours < 1 &&
                !string.IsNullOrEmpty(LastValue)) return LastValue;
        ...
        {
            Log.Error(
                "[CURRENTCLOCKSPEED] [{name}] Error getting current clock speed: {msg}",

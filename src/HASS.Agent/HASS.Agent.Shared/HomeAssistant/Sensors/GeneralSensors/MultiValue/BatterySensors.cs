...
/// <summary>
/// Multi-value sensor containing power and battery info
/// </summary>
public class BatterySensors : AbstractMultiValueSensor
{
    ...
    public sealed override void UpdateSensorValues()
    {
        ...
        var chargeStatusSensor = new DataTypeStringSensor(
            _updateInterval, 
            chargeStatusEntityName, 
            "Charge Status", 
            chargeStatusId, 
            string.Empty, 
        ...
        if (fullChargeLifetimeMinutes != -1)
            fullChargeLifetimeMinutes = Convert.ToInt32(
                Math.Round(TimeSpan.FromSeconds(fullChargeLifetimeMinutes)
                    .TotalMinutes)
            );
        ...
        var fullChargeLifetimeSensor = new DataTypeIntSensor(
            _updateInterval, 
            fullChargeLifetimeEntityName, 
            "Full Charge Lifetime", 
        ...
        var chargeRemainingPercentage = Convert.ToInt32(
            powerStatus.BatteryLifePercent * 100);
        var chargeRemainingPercentageEntityName = 
            $"{parentSensorSafeName}_charge_remaining_percentage";
        var chargeRemainingPercentageId = $"{Id}_charge_remaining_percentage";
        var chargeRemainingPercentageSensor = new DataTypeIntSensor(
            _updateInterval, 
            chargeRemainingPercentageEntityName, 
            "Charge Remaining Percentage", 
            chargeRemainingPercentageId, 
            string.Empty, 
            "measurement", 
            "mdi:battery-high", 
            "%", 
        ...
        AddUpdateSensor(chargeRemainingPercentageId, 
            chargeRemainingPercentageSensor);

        var chargeRemainingMinutes = powerStatus.BatteryLifeRemaining;
        if (chargeRemainingMinutes != -1)
            chargeRemainingMinutes = Convert.ToInt32(
                Math.Round(TimeSpan.FromSeconds(chargeRemainingMinutes)
                    .TotalMinutes)
            );
        ...
        var chargeRemainingMinutesSensor = new DataTypeIntSensor(
            _updateInterval, 
            chargeRemainingMinutesEntityName, 
            "Charge Remaining", 
            chargeRemainingMinutesId, 
            string.Empty, 
            "measurement", 
            "mdi:battery-high", 
            string.Empty, 
        ...
        var powerlineStatusSensor = new DataTypeStringSensor(
            _updateInterval, 
            powerlineStatusEntityName, 
            "Powerline Status", 
            powerlineStatusId, 
            string.Empty, 
            "mdi:power-plug", 

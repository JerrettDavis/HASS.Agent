...
/// </summary>
public class CurrentVolumeSensor(
    int? updateInterval = null,
    string? entityName = CurrentVolumeSensor.DefaultName,
    string? name = CurrentVolumeSensor.DefaultName,
    string? id = default,
    string? advancedSettings = default)
    : AbstractSingleValueSensor(entityName ?? DefaultName, name ?? null,
        updateInterval ?? 15, id, advancedSettings: advancedSettings)
{

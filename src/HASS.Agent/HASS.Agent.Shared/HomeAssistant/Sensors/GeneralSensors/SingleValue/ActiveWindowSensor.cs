...
public class ActiveWindowSensor : AbstractSingleValueSensor
{
    ...
    public override DiscoveryConfigModel? GetAutoDiscoveryConfig()
    {
        ...
        return AutoDiscoveryConfigModel ??
            SetAutoDiscoveryConfigModel(new SensorDiscoveryConfigModel
            {
                ...
                Json_attributes_topic = $"{Variables.MqttManager.MqttDiscoveryPrefix()}/" +
                                        $"{Domain}/{deviceConfig.Name}/{ObjectId}/attributes"
            });
    }
    ...
    public override string GetState()
    {
        ...
        if (returnValue == 0 || processId == 0) return GetWindowTitle(windowHandle);
    ...
    public override string GetAttributes() =>
        JsonConvert.SerializeObject(new { processName = _processName });
    ...
    private static extern int GetWindowText(
        IntPtr hWnd, StringBuilder builder, int count);
    ...
    private static string GetWindowTitle(IntPtr windowHandle)
    {
        ...
        var windowTitle = GetWindowText(windowHandle, builder, titleLength) > 0 
            ? builder.ToString() 
            : string.Empty;

        // Note: to ensure we don't exceed HA limitation of 255 payload length
        return windowTitle.Length > 255 
            ? windowTitle[..255] 
            : windowTitle;
    }
}